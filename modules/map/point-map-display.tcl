...
SCRATCH ....................... AKIS pieces for point display


	canvas::edit points ${selfns}::POI $win.map \
	    -data-cmd      [mymethod DATA] \
	    -create-cmd    [mymethod NEW]  \
	    -highlight-cmd [mymethod HIGHLIGHT]

	${selfns}::POI disable drag edit ;# keep highlight on

---

!! create command !!
    variable lastid
    method DATA {op c item args} { set lastid $item }

    method NEW {c x y} {
	debug.ak/is/app/tk/map {}

	lappend items [$c create image $x $y -image $poi -anchor s]
	return $items
    }

NOTE: -anchor -- icon dependent --
	-- poi		s/south
	-- feature	s/south
	-- group	c/center

    method HIGHLIGHT {op c detail} {
	debug.ak/is/app/tk/map {}

puts $op/$c/((($detail)))

	switch -exact -- $op {
	    on  { $c itemconfigure $detail -image $poihi ; return $detail }
	    off { $c itemconfigure $detail -image $poi }
	}
	return
    }
...
--- move to x,y
	    ...
	    lassign $pix x y
	    lassign [dict get $mloc $mark] ox oy

	    set dx [expr {$x - $ox}]
	    set dy [expr {$y - $oy}]
	    # Move to proper position
	    ${selfns}::POI move-by $mark $dx $dy

	    dict set mloc $mark $pix

---

	# remember position for future movement
	dict set mloc $mark $pix

---

	    # Create a new marker at the desired location
	    ${selfns}::POI add {*}$pix ;# -> DATA callback
	    set mark $lastid

---

	# remove tool tip, image - latter makes marker invisible
	tooltip::tooltip $win.map -items $mark -- {}
	$win.map itemconfigure $mark -image {}
...
pathName bbox tagOrId ?tagOrId tagOrId ...?
	...
	Returns a list with four elements giving an approximate bounding box for all the items named
        ...
        all the named elements are within the region bounded by x1 on the left, x2 on the right, y1
        on the top, and y2 on the bottom.  The return value may overestimate the actual bounding box
        by a few pixels.  If no items match any of the tagOrId arguments or if the matching items
        have empty bounding boxes (i.e. they have nothing to display) then an empty string is
        returned.
