...
#
##  <obj> ids			-> list (id...)
##  <obj> get ID		-> dict (name -> STRING, geo -> GEOBOX)
...
snit::type ::map::box::store::fs {
	...
	foreach path [glob -nocomplain -directory $directory $options(-pattern)] {
		...
		set d [read $c]
		close $c
		set box [lrange [lmap line [split $d \n] {
		    set line [string trim $line]
		    if {$line eq {}} continue
		    if {![string is double -strict $line]} continue
		    set line
		}] 0 3]
	    } on error {e o} {
		debug.tklib/map/box/store/fs { $path - $e }
		#puts $path:\t$e
	    }
	    ...
	    foreach p [map slippy geo box corners $box] {
		dict set mypoints $p $path
	    }

	    # Update base attribute information
	    dict set attr name [file root [file tail $path]]
	    dict set attr geobox $box

	    dict set myattr $path $attr
	}
