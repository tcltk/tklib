...
If you are using a Mac, one of the sets of radiobuttons at the bottom of the window will not be disabled (this set of buttons is not present on non-Mac platforms).  These radiobuttons allow you to set the value of ::ntext::strictAqua, which controls the response to the keyboard events described in (4), (5) below.
...
(3) Up and Down keys, with Command modifier
    ...
    On other platforms these operations are bound to the keys <Control-Home>, <Control-Shift-Home>, <Control-End>, <Control-Shift-End>.

(4) Up and Down keys, with Control modifier

The effect of these keys depends on the value of ::ntext::strictAqua.

if {::ntext::strictAqua == 1} (the default value)

    The bindings attempt to do what other Mac applications do.

    <Control-Up>, <Control-Down> do the same as <Prior>, <Next> respectively; they do not move the insert mark.

    <Shift-Control-Up>, <Shift-Control-Down> do nothing.

if {::ntext::strictAqua == 0}

    The behavior differs from that of other Mac applications.

    The bindings do the same on the Mac as on other platforms.  This is also the same as the traditional Tk Text widget on the Mac.

    <Control-Up>, <Control-Down> scroll the text widget up and down by "paragraph", AND move the insert mark.

    <Shift-Control-Up>, <Shift-Control-Down> scroll the text widget up and down by "paragraph", AND move the insert mark, AND extend the selection.
...
if {::ntext::strictAqua == 0}
    ...
    <Option-Up>         does the same as <Control-Up>
    <Option-Down>       does the same as <Control-Down>
    <Shift-Option-Up>   does the same as <Shift-Control-Up>
    <Shift-Option-Down> does the same as <Shift-Control-Down>
