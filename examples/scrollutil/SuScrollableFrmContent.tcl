...
grid $l -row $row -column 0 -columnspan 3 -sticky w -padx 9p -pady {9p 0}
incr row
set _sa [scrollutil::scrollarea $cf.sa$row]
set txt [text $_sa.txt -font TkFixedFont -width 73]
scrollutil::addMouseWheelSupport $txt
$_sa setwidget $txt
grid $_sa -row $row -column 0 -columnspan 3 -sticky w -padx 9p -pady {3p 0}
...
grid $l -row $row -column 0 -sticky w -padx {9p 0} -pady {9p 0}
incr row
set _sa [scrollutil::scrollarea $cf.sa$row]
set lb [listbox $_sa.lb -width 0]
$_sa setwidget $lb
grid $_sa -row $row -rowspan 6 -column 0 -sticky w -padx {9p 0} -pady {3p 0}

#
# A ttk::combobox widget
#
set l [ttk::label $cf.l$row -text "Release:"]
grid $l -row $row -column 1 -sticky w -padx {9p 0} -pady {3p 0}
set cb [ttk::combobox $cf.cb -state readonly -width 14]
bind $cb <<ComboboxSelected>> updateWidgets
grid $cb -row $row -column 2 -sticky w -padx {3p 9p} -pady {3p 0}
...
grid $l -row $row -column 1 -sticky w -padx {9p 0} -pady {9p 0}
set sb [ttk::spinbox $cf.sb -style Wide.TSpinbox -from 0 -to 20 \
	-state readonly -width 4]
grid $sb -row $row -column 2 -sticky w -padx {3p 9p} -pady {9p 0}
...
grid $e -row $row -column 2 -sticky w -padx {3p 9p} -pady {9p 0}
...
grid $sep -row $row -column 1 -columnspan 2 -sticky we -padx 9p -pady {9p 0}
...
grid $me -row $row -column 2 -sticky w -padx {3p 9p} -pady {9p 0}
...
$_sa setwidget $tv
grid $_sa -row $row -column 0 -columnspan 3 -sticky w -padx 9p -pady {3p 9p}
...
proc configTablelist tbl {
    ...
    set sf [scrollutil::scrollableframe $sa.sf -fitcontentwidth yes]
    $sa setwidget $sf
